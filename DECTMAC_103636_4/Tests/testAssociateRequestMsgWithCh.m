...
phy_Control_Field2.DF_MCS = 11;                  % 4 bits. PDC MCS. 
...
% associate_request_msg beacon ie
...
[association_request_msg_ie] = association_request_msg_ie_constr(setup_cause,number_of_flows,Power_const, FT_mode, Current, HARQ_process_TX, MAX_HARQ_Re_TX, HARQ_process_RX, MAX_HARQ_Re_RX,...
...
mac_packet_info = [mac_header_type mac_common_header association_request_msg_ie rd_capability_msg_ie];
...
% modulation 
% pdc_soft_symbols = 2*pdc_enc_data-1;
...
%%
phy_msg_bin = dectPCCSerializer(phy_Control_Field1);

%len8 = length(rx_mac_sdus)/8;
%mac_msg_bin = uint8(bi2de(fliplr((reshape(rx_mac_sdus,8,len8)'))))';
mac_msg_bin = mac_msg_serializer(rx_mac_sdus);

msg_out = uint8([phy_msg_bin mac_msg_bin]);
% % 
% % %msg_out(1:length(phy_msg_bin)) = phy_msg_bin;
% % %msg_out((length(phy_msg_bin)+1):(length(phy_msg_bin)+length(mac_msg_bin))) = mac_msg_bin;
% % %msg_out = uint8(msg_out);
% % msg_out = uint8(mac_msg_bin);
...
write(u,msg_out,"uint8",ADDRESS,PORT);
clear u
