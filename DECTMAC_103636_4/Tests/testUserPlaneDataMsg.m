...
%
% 103634-4 
% send random data that has the max packet size 
% MAC Header Type 0000
% Data MAC PDU Header 6.3.3.2
% Data with header 
...
phy_Control_Field1.Packet_Length_Type = 0;      % 1 bit. length given in slots 
...
phy_Control_Field2.Packet_Length_Type = 0;      % 1 bit. length given in slots 
...
for i1 =1:16
phy_Control_Field.phy_Packet_Length = i1-1;
...
phy_Control_Field.phy_Packet_Length = 15;
[ind_pdc]   = dectPDCind(phy_Control_Field,transmission_modes,mu_beta);
N_PDC_re = length(ind_pdc);
[N_TB_bits] = dectTransportBlockSize(N_PDC_re,phy_Control_Field,transmission_modes,mu_beta);
max_bits_per_symb = N_TB_bits;

%% Broadcasted Cluster header 
% MAC Header Type: DATA MAC PDU as in 6.3.3.1-1
...
% get packet size 
mac_packet_len_target = max_bits_per_symb;
% mac header type: Beacon header
version = 0;
mac_security = 0;  % not used 
beacon_header = 0; % DATA MAC PDU header
[mac_header_type] = mac_MAC_Header_Type_constr(version,mac_security,beacon_header);

% MAC Common Header: DATA MAC PDU Header 6.3.3.1
% mac common header: DATA MAC PDU Header 
Reset = 0;
sequence_number = 1;
[mac_common_header] = mac_Common_Header_DATA_MAC_PDU_Header_constr( Reset, sequence_number);

mac_packet_info = [mac_header_type mac_common_header];

headers_length = length(mac_packet_info);
% data length computation
available_data_and_header_length = mac_packet_len_target - headers_length;
if available_data_and_header_length <= (256 + 16);
available_data_length = available_data_and_header_length - 16;
else
available_data_length = available_data_and_header_length - 24;
end

rand_data = randn(1,available_data_length)>0;

% generate data frame ie
flow_nr = 1;
[data_mac_pdu_ie] = user_plane_data_flow_ie_constr(flow_nr,rand_data,available_data_length);

mac_packet_info = [mac_packet_info data_mac_pdu_ie];
...
h2 = zeros(size(rx_frame));
h2(1:length(stf)) = stf;
h2fc= conj(fft(h2));

% correlation search
tmpCorr = abs(ifft(fft(rx_frame).*h2fc));

% data extraction 
[v,loc1] = max(tmpCorr);

startLoc = loc1 + 112; % +1 due to the matlab starting at inx 1

%% pcc reception

% decoding 
pcc_symbols_samples = rx_frame((loc1+111)+[1:3*(mu_beta.N_DFT+mu_beta.N_cp)]);

[dect_pcc_grid_rx,info] = dectOFDMDemodulate(pcc_symbols_samples,mu_beta);

phy_Packet_Length = 1;
packet_Length_Type = 0;

[h_est] = dectPCCSymbolsChannelEstimate(dect_pcc_grid_rx,transmission_modes,mu_beta);
[dect_pcc_grid_rx_eq] = dectPCCSymbolsZeroForcing(dect_pcc_grid_rx,h_est,transmission_modes,mu_beta);

[ind_pcc]   = dectPCCind(transmission_modes,mu_beta);

pcc_re_symbols = dect_pcc_grid_rx_eq(ind_pcc);
pcc_soft_symbols = dectSymbolDeMapping(pcc_re_symbols,1);
[pcc_Phy_Control_Field_rx] = dectPCCDecode(pcc_soft_symbols);

%% data demapping 
% how many OFDM symbols
N_PACKET_symb = 10;
if pcc_Phy_Control_Field_rx.Packet_Length_Type == 0
  N_PACKET_symb = pcc_Phy_Control_Field_rx.phy_Packet_Length*mu_beta.N_SLOTmu_symb/mu_beta.N_SLOTmu_subslot;
else 
  if pcc_Phy_Control_Field_rx.Packet_Length_Type == 1
    N_PACKET_symb = pcc_Phy_Control_Field_rx.phy_Packet_Length*mu_beta.N_SLOTmu_symb;
  end
end
N_PACKET_symb = N_PACKET_symb -2;
pcc_symbols_samples = rx_frame((loc1+111)+[1:N_PACKET_symb*(mu_beta.N_DFT+mu_beta.N_cp)]);

[dect_grid_rx,info] = dectOFDMDemodulate(pcc_symbols_samples,mu_beta);

%% pdc extraction
% equalization
[h_est] = dectChannelEstimate(dect_grid_rx,pcc_Phy_Control_Field_rx,transmission_modes,mu_beta);
[dect_grid_rx_eq] = dectZeroForcing(dect_grid_rx,h_est,pcc_Phy_Control_Field_rx,transmission_modes,mu_beta);
[ind_pdc]   = dectPDCind(pcc_Phy_Control_Field_rx,transmission_modes,mu_beta);

%% pdc decoding 
% data demapping 
N_PDC_re = length(ind_pdc);
pdc_re_symbols = dect_grid_rx_eq(ind_pdc);
MCS = pcc_Phy_Control_Field_rx.DF_MCS;
pdc_soft_symbols = dectSymbolDeMapping(pdc_re_symbols,MCS);
[pdc_MACSDU] = dectPDCDecode(pdc_soft_symbols, NetworkID, N_PDC_re, pcc_Phy_Control_Field_rx,transmission_modes,mu_beta);
...
switch(rx_mac_header_type_struct.MAC_header_type)
  case 0
    ...
    [ Reset, sequence_number] = mac_Common_Header_DATA_MAC_PDU_Header_extr(rx_mac_packet(8+[1:2*8]));
    rx_mac_sdus = rx_mac_packet([(3*8+1):end]);
  case 1
